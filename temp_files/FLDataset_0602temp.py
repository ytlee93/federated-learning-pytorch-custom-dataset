...
class CovidDataset(Dataset):
    def __init__(self, csv_path, transform=None):
        ...
        print("In covidDataset init len data_info = ", len(self.data_info))
        self.image = []
        self.label = []
        for i in range(len(self.data_info)):
            img_name = self.data_info.iloc[i, 0]
            img = cv2.imread(img_name, cv2.IMREAD_GRAYSCALE)
            img = cv2.resize(img, (32,32))
            lb = self.data_info.iloc[i, 1]
            lb = np.array([lb])
...
#         print("get item image shape: ", self.image[idx].shape)
